...
public class PayPalController {
     ...
     * Body: { "amount": 29.0, "currency": "USD", "transactionRef": "TXN-XXX" }
     */
    ...
    public ResponseEntity<?> captureOrder(@PathVariable String orderId) {
        try {
            ...
            if (response.getStatusCode() == HttpStatus.CREATED && response.getBody() != null) {
                ...
                resp.put("orderId", orderId);
                resp.put("status", result.get("status"));
                resp.put("captureId", extractCaptureId(result));
                resp.put("payerEmail", extractPayerEmail(result));
    ...
    public ResponseEntity<?> getOrderDetails(@PathVariable String orderId) {
        try {
            ...
            ResponseEntity<Map> response = restTemplate.exchange(
                getApiUrl() + "/v2/checkout/orders/" + orderId,
                HttpMethod.GET,
                request,
                Map.class
            );
    ...
    // Helper methods
    private String extractCaptureId(Map<?, ?> result) {
        try {
            List<?> purchaseUnits = (List<?>) result.get("purchase_units");
            if (purchaseUnits != null && !purchaseUnits.isEmpty()) {
                Map<?, ?> unit = (Map<?, ?>) purchaseUnits.get(0);
                Map<?, ?> payments = (Map<?, ?>) unit.get("payments");
                if (payments != null) {
            ...
            System.err.println("[PayPal] Error extracting capture ID: " + e.getMessage());
        }
        return null;
    }

    private String extractPayerEmail(Map<?, ?> result) {
        try {
            Map<?, ?> payer = (Map<?, ?>) result.get("payer");
            if (payer != null) {
                return (String) payer.get("email_address");
            }
        } catch (Exception e) {
            System.err.println("[PayPal] Error extracting payer email: " + e.getMessage());
        }
        return null;
    }
}
